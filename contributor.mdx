...
<Accordion title="Steps to Add a New Game">
To add a new game to the project, follow these steps:

### Steps:
1. **Create Game-Specific Files**:
   - Add a new folder in the `engine/` directory for your game logic.
   - Implement game-specific systems, components, and logic.

2. **Register the Game with the Engine**:
   - Modify the engine's main configuration to include the new game logic.

   **Example**:
   ```cpp
   void Engine::loadNewGame() {
       // Initialize game-specific systems and components
   }
   ```

3. **Test Gameplay**:
   - Ensure the game runs correctly and integrates with the existing engine.

4. **Commit Changes**:
   - Follow the project ruleset for pull requests and approvals.
</Accordion>
...
2. Creating new games and integrating them with the engine.
3. Adding new TCP and UDP commands for the server.
